...
프로젝트 하나를 **레퍼런스 자료 한 벌**로 번역한다 — 기술 도면 · 비전문가용 사용법 도면 ·
LinkedIn 캐러셀 · Upwork 포트폴리오 항목 · 케이스 스터디 · 이력서 bullet.
푸는 문제는 *"프로젝트는 많이 하는데 매번 까먹어서 나중엔 레퍼런스를 못 만든다"* 이다.

| 스킬 | 명령 | 역할 |
|---|---|---|
| kai-showcase | `/kai-showcase [프로젝트경로] [--quick\|--text-only]` | 분석 → 질문 6개 → 도면·캐러셀·텍스트 → `<프로젝트>/showcase/` |

**제1원칙: 코드가 답할 수 있는 것은 묻지 않는다.** 스택·규모·기능·아키텍처·배포·기간과
"왜 이렇게 했는가"(CLAUDE.md·핸드오프·긴 커밋 메시지)를 먼저 전부 캐내 초안을 채우고,
사람만 아는 **6개**(왜 시작 / 내 역할 / 고비 / 결과 수치 / 공개 범위 / 노리는 자리)만 묻는다.
무응답은 `🔶` 로 남기고 **그대로 산출** — 미완이라도 남기는 것이 목적이다.

산출 언어는 **영어 주 · 한국어 병기**(각 절 끝 `> 🇰🇷` 인용블록), 캐러셀은 영어만.
채널 규격은 실측 근거로 고정: LinkedIn **1080×1350(4:5) 6~10장**, Upwork **8초 스캔 · 전 필드**,
리크루터 **Problem→Approach→Process→Artifacts→Impact→Learnings + 레이어별 트레이드오프**.

`assets/capture.sh` 는 HTML 을 PNG 로 굽는 하네스다. 2026-09-15 에 실제로 밟은 함정 6가지가
전부 방어책으로 굳어 있다 — Playwright `file:` 차단(Chrome 직접) · **Chrome 미종료(--timeout +
PID kill + 폴링 삼중 방어)** · 사용자 Chrome 충돌(프로필 분리) · fullPage 부재(Pillow 트림) ·
다크모드/애니메이션/lazy 이미지(래퍼가 고정) · 한국어 조사 줄바꿈(`word-break: keep-all`).
실측: page ~8초 · fixed ~6초 · 1080×1350 → **2160×2700**.

설치:
```bash
bash /Volumes/KAIFACUN/Projects/Skills/showcase/install.sh
```
...
10-b. **kai-gen MCP 교차 검증 (bash Codex CLI 대체)** — 리스크 분석은 bash CLI·`.plans/` 중간 파일 없이 **LLM이 `mcp__kai-gen__kai_consult` 를 직접 호출**: add 등록 에이전트는 Tier 3 스펙(`· 등록` 섹션)을, run 워커는 tier≥2 구현 플랜(`· 구현` 섹션)을 반박 우선 검증(단일 호출·4부 압축 context·최대 10분 허용·거절한 치명 지적도 기록). 실패는 기록 후 자체 플랜 진행(soft-fail), 성공·승계 기록만 멱등 skip. claim 스크립트는 순수 파일 연산으로 복귀(Step 2 블로킹 해소)
10-c. **add↔run 중복 노동 방지 (스펙-코드 일치 분기)** — 계획은 add(지침서)의 몫, run 워커의 몫은 실행. 워커는 스펙-코드 일치 시 **등록 스펙을 그대로 플랜으로 채택**(재수립 금지, `PLAN_SOURCE=spec`)하고, 등록 검증까지 있으면 구현측 kai_consult도 **승계·생략**(`- 생략: 등록 검증 승계` 기록 의무). 재수립·재검증은 등록 검증 부재·스펙-코드 불일치·플랜 변경 시에만
