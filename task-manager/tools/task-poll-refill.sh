#!/bin/bash
# task-poll-refill.sh — Step 5 (슬롯 리필판): 추적 중인 워커 중 "하나라도 종료되면 즉시 반환"
# Usage: task-poll-refill.sh <SESSION_ROOT> <f1> [f2] ...
...
#   TIMEOUT:<f>   MAX 동안 아무도 안 끝나 timeout 처리됨
#
# 배리어판(task-poll-and-attach.sh)과 차이:
#   - 배리어: PENDING=0(배치 전부)까지 대기.
#   - 리필  : pending < 진입시(=하나라도 완료) 즉시 반환 → 빈 슬롯을 run이 곧바로 리필.
