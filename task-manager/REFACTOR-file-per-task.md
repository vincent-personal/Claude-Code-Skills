...
- **완료 무결성 보강(후속 수정)**: 실사용에서 "커밋했으나 `mv doing→done` 누락" 사고가 관찰되어 6가지 보강 — ①완료 mv를 보고보다 먼저·완료 정의 행위로 ②Step 6 모순(blocked vs done) 제거 ③Step 7-V 완료 검증 ④Red Line(doing 잔류 금지) ⑤커밋 직후 `committed:` 해시 마커 ⑥Step 0-B 화해 분기(committed 있으면 done으로, 재실행 금지). → 누락이 나도 다음 run이 **중복 커밋 없이** done/ 으로 화해.
- **병합(Step 1-A)은 best-effort**: 두 add가 거의 동시에 실행되면, A의 todo/ 스캔 시점에 B의 신규 파일이 아직 보이지 않아 병합을 놓치고 같은 범위 작업이 todo 파일 2개로 생길 수 있다. **무해**(손상·유실 없음 — staging→원자적 mv라 스캔이 반쯤 쓰인 파일을 읽는 일은 없다). 두 파일은 impact_files가 겹쳐 충돌 검사가 동시 실행을 막으므로 순차 처리된다. 완전 차단은 add 전역 락 필요 → 경합 재도입이라 의도적으로 미채택.
