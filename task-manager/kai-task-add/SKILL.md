...
### Step 1 — 등록 서브에이전트 위임 (백그라운드 · 컨텍스트 격리 · 필수)
...
> 🚀 **백그라운드 등록 (add 병렬화)**: Agent는 `run_in_background: true` 로 스폰하고, 메인 세션은 스폰 직후
> "등록 진행 중" 한 줄만 보고하고 **턴을 즉시 종료**한다. 큐에 쌓인 다음 `/kai-task-add` 가 곧바로 처리되어
> 등록(특히 Tier 3 advisor 자문 수 분)이 겹쳐 돌아간다. 안전 근거:
> ① **FIFO 보존** — TASK_ID는 Step 0-F에서 턴 시작 시 이미 발급됨 → 완료 순서와 무관하게 파일명 정렬 = 명령 순서.
> ② **미완성 보호** — staging→원자적 mv + `ready:false` 게이트로 task-run이 착수하지 않음.
> ③ **고아 복구** — 등록 에이전트가 죽어 `ready:false` 로 30분+ 방치되면 task-run Step 0-A sweep이 blocked 처리.
...
**F-2. kai-gen 교차 검증 (Tier 3 · advisor 반영 직후 · 1회):**
advisor 반영이 끝난 **스펙**(scope·impact_files·배선·체크리스트)을 타 계열 모델에게 반박 우선 검증받는다.
- ToolSearch 1회: `select:mcp__kai-gen__kai_consult` (도구가 안 보이면 이 단계 전체 skip — 등록은 계속 진행, 섹션 생략).
- `context` = ① 작업 설명 요약 ② 확정 impact_files 목록 ③ 구현 방안·체크리스트. **탐색한 파일 원문 덤프 금지** (과대 컨텍스트 = abort 원흉).
- `question` = "위 자료는 다른 AI가 확정하려는 작업 스펙이다. 동의하려 하지 말고 먼저 반박을 시도하라. 특히 **누락된 영향 파일·호출 트리거(배선)·선행조건**을 찾아라. 반박에 실패한 지점만 '검증됨'으로 표시하라." / `effort` = "medium".
- ⏱ 최대 10분 소요 정상 — 백그라운드 전환 시 결과 알림을 기다린다. 실패 시 재시도 1회 후 skip.
- 유효 지적 → impact_files·구현 방안·체크리스트에 반영(오탐은 무시 — 최종 판단은 등록 에이전트). task 본문에 기록:
  ```markdown
  ## 교차 검증 (kai-gen · 등록)
  - 모델: {usedModel}
  - 수용: {수용 지적 2~3줄. 없으면 "지적 없음"}
  - 거절: {거절한 치명 지적 + 사유 1~2줄. 없으면 생략}
  ```
※ 구현 단계 검증(워커의 `· 구현` 섹션)과 역할이 다르다 — 여기서는 **스펙 완전성**만 본다.
...
**스폰 직후 (같은 턴):**
"⏳ 등록 진행 중 — id={TASK_ID}" 한 줄만 보고하고 **턴을 즉시 종료**한다. 등록 완료를 포그라운드로 기다리지 않는다.

**완료 알림(task-notification) 수신 시:**
출력의 `RESULT:` 에서 `impact_files=` 필드가 없으면 → 실패로 간주하고 사용자에게 오류 보고.
있으면 → `RESULT:` 한 줄을 사용자에게 보고하고 종료한다.
(알림 전에 사용자가 세션을 닫아도 무결성은 ready 게이트·Step 0-A sweep이 지킨다 — 보고만 생략될 뿐이다.)
...
> ℹ️ **동시 등록 한계 (허용된 트레이드오프)**: add는 다중 세션 + 백그라운드로 겹쳐 돌므로,
> 아직 todo/ 에 나타나지 않은 다른 등록의 작업은 스캔에 안 보인다 — 같은 범위 작업이 병합되지 못하고
> 중복 생성될 수 있다. 단 impact_files 가 겹치는 중복 task는 task-run 선점 시 doing/ 교집합 검사가
> 동시 실행을 차단해 직렬화되므로, 최악의 경우도 "뒤 워커가 이미 반영된 것을 확인하고 지나감"에 그친다.
...
- ❌ **등록 Agent를 포그라운드(동기)로 대기** — 반드시 `run_in_background: true` 스폰 후 턴 종료. RESULT 검증·보고는 완료 알림 수신 턴에서 수행 (큐의 후속 add를 막지 않는다)
