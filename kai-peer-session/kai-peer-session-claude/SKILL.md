...
description: |
  ...
  트리거: /kai-peer-session-claude 명시 호출 외에도, 사용자가 Claude(클로드) 세션의 작업을
  참조·숙지·이어받으라는 취지로 말하면 스킬 이름 없이 이 스킬을 쓴다. 예:
  "클로드로부터", "클로드로부터 받아서", "클로드가 한 작업 참조해서",
  "클로드 쪽에서 뭐 했는지 봐줘", "클로드 작업 이어서 해줘", "짝 세션 확인해줘".
  방향 주의: Claude Code 세션에서 kai-gen 작업을 읽는 반대 방향은 kai-peer-session-kaigen.
---
